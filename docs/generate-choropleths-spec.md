# Synthetic Choropleth Generator for Gemma 4 Testing
...
- Tests: flat color reading without prior knowledge interference

### Chart 3: Circle Size Only (no color variation)
- 12 zones, all same background color (light gray)
- Circles overlaid: 3 sizes (small=cheap, medium=mid, large=expensive)
- Legend: "Prix médian au m²" with 3 circle sizes
- Tests: pure size encoding reading

### Chart 4: Color + Circle Size (two dimensions)
- 12 zones with political bloc colors (Chart 1 style)
...
- Tests: gradient direction reading, does lighter = cheaper register correctly?

### Chart 6: Color Gradient + Labels
- Same as Chart 5 but with price labels printed inside each zone
- Tests: does printed number help or does Gemma still mis-read gradient direction?
...
matplotlib
numpy
matplotlib-hexbin (built into matplotlib)
```

### Output Format
- PNG images, 1200x900px
- Save to `test_charts/choropleths/` directory
- Filename: `choropleth_{id}_{description}.png`
...
```
  ...
  D   E   F   G
    H   I   J
      K   L
```
12 zones in offset grid pattern.

### Manifest Format
```json
{
  "choropleth_01_political.png": {
    "zones": [
      {
        "name": "Zone A",
        "category": "Gauche",
...
|-------|---------------|
| Color-to-category mapping | Did it match legend colors to correct zones? |
| Prior knowledge override | Did it swap political colors (left=blue, right=red)? |
| Circle size reading | Did it identify small/medium/large correctly? |
| Two-dimension independence | Did it separate color meaning from circle meaning? |
| Gradient direction | Did it read lighter=cheaper correctly? |
| Printed label accuracy | Did explicit numbers help vs gradient-only? |
